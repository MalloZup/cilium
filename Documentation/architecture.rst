...
The following is a list of the hooks used by a Cilium and a brief
...
* **Traffic Control Ingress:** BPF programs attached to the traffic control (tc)
  ...
  By attaching a BPF program to the veth pair from all containers and routing
  all network traffic to a host side virtual device with another BPF program attached to
...
* **Socket operations:** The socket operations hook is attached to a specific
  ...
  a socket transitions into ESTABLISH state if the TCP socket has a node
...
* **Prefilter:** The prefilter object runs an XDP program for best performance and
  provides a set of prefilter rules used to filter traffic from the network. Specifically,
  a set of CIDR maps supplied by the Cilium agent are used to do a lookup and the packet
  is either dropped or allowed to be processed by the stack. This can be easily
...
* **Socked Layer Enforcement:** Socket layer enforcement use two
...
* **L7 Policy:** The L7 Policy object redirect proxy traffic to a Cilium userspace
  ...
  then either forward the traffic or drop the traffic based on the configured L7 policy.
...
there is an additional diagram showing the TCP accelerated path available when Socket
Layer Enforcement is enabled.
...
traffic the
handshake initiating the connection will traverses the endpoint policy object until TCP state
...
This complete the datapath overview. More BPF specifics can be found in the
BPF section, :ref:`bpf_guide`. Additional details on how to extend the L7 Policy
exist in the Envoy section, :ref:`envoy`
