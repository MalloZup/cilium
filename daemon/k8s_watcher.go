	...
	svcController := k8sUtils.ControllerFactory(
		...
		k8sUtils.ResourceEventHandlerFactory(
			...
			func(m versioned.Map) versioned.Map {
				return d.k8sSvcCache.ListMissingServices(m)
			},
			&v1.Service{},
			k8s.Client(),
			reSyncPeriod,
	...
	for {
		...
		case k8s.UpdateService, k8s.UpdateIngress:
			scopedLog.WithFields(logrus.Fields{
				logfields.ServiceIP: event.Service.FrontendIP,
				"selector":          event.Service.Selector,
			}).Debug("Service definition has changed")
			...
			if svc.IsExternal() {
				serviceImportMeta, cacheOK := endpointMetadataCache.get(event.K8sEndpoints)
				...
				if !cacheOK || (cacheOK && serviceImportMeta.ruleTranslationError != nil) {
					...
					endpointMetadataCache.upsert(event.K8sEndpoints, err)
					if err != nil {
		...
		case k8s.DeleteService, k8s.DeleteIngress:
			...
			if svc.IsExternal() {
				endpointMetadataCache.delete(event.K8sEndpoints)
...
func (d *Daemon) updateK8sEndpointV1(oldEP, newEP *v1.Endpoints) error {
	log.WithFields(logrus.Fields{
		...
		logfields.K8sAPIVersion:   newEP.TypeMeta.APIVersion,
	}).Info("Updated  Kubernetes Endpoints")
...
func (d *Daemon) deleteK8sEndpointV1(ep *v1.Endpoints) error {
	log.WithFields(logrus.Fields{
		...
		logfields.K8sNamespace:    ep.ObjectMeta.Namespace,
		logfields.K8sAPIVersion:   ep.TypeMeta.APIVersion,
	}).Info("Deleted Kubernetes Endpoints")
...
func (d *Daemon) missingK8sEndpointsV1(m versioned.Map) versioned.Map {
	return d.k8sSvcCache.ListMissingEndpoints(m)
}
